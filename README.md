...
The product page follows the supplied Etsy layout: centered collection breadcrumbs, a large image with a left thumbnail rail on desktop, horizontal thumbnails on mobile, arrows, swipe/scroll, keyboard navigation, and an image zoom dialog. Product media and their order come from the product admin page. Image ratio, fit, position, quantity, save controls, and checkout buttons are editable under **Customize → Products → Product**. The local fixture omits accelerated checkout; validate checkout and real video/model media in a Shopify development theme.

The outlined **Buy it now** button appears above Add to cart by default. It submits the selected variant, quantity, and personalization through Shopify's [native product form with a checkout return path](https://shopify.dev/docs/api/liquid/tags/form#form-return_to). It includes the existing cart and uses normal form validation. Sold-out products and pending variant updates disable it; an active add-to-cart request also disables it. Add to cart keeps the existing toast-only behavior. Switch off **Show Buy it now button** to use the optional Shopify accelerated checkout button instead. The local Buy it now fixture only echoes submitted values on a marked checkout preview and never creates an order or processes payment.
